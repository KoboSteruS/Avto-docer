        ...
        try:
            ...
            if media_groups:
                ...
                for group_id, messages in media_groups.items():
                    ...
                    logger.info(f'   ✅ Группа из {len(all_photos)} фото → 1 пост')
                ...
                for idx, message in enumerate(batch, 1):
                    try:
                        ...
                        if not text and not photos:
                            logger.warning(f'   ⚠️  Пост #{idx}: Нет текста и фото, пропускаем')
                            skipped_count += 1
                            continue
                        ...
                        if text:
                            logger.info(f'   📰 Пост #{idx}: {title[:50]}... (📝 текст, 📷 {len(photos)} фото)')
                        else:
                            logger.info(f'   📷 Пост #{idx}: {title[:50]}... (только фото: {len(photos)} шт.)')
